...
- `ui/index.html` — the entire web UI (**source lives here, not under `data/`**): a single-file React app loaded via UMD `<script>` tags with in-browser Babel (`type="text/babel"`), split into several `<script type="text/babel">` IIFE blocks (hooks/primitives, shared widgets, tab components). No JS bundler — edit this file directly and it's transpiled client-side on load. It is gzipped into `data/index.html.gz` by `scripts/gzip_ui.py`, which `platformio.ini` runs automatically whenever the filesystem image is built (221KB -> 62KB; the uncompressed copy is deliberately NOT under `data/` so it cannot ship). The device serves the `.gz` and `streamFile()` sets `Content-Encoding` itself. **Each `<script type="text/babel">` block is its own JS scope** — a top-level `const` declared in one block is not visible in another (bit us twice already: two same-named but differently-shaped `COLORS` arrays in different blocks, see `doc/content/history.md` 2026-08-15). When adding a shared constant, check which block actually needs it rather than assuming it's globally visible.
...
- The C3 has **no hardware FPU** (RV32IMC) — every `sinf`/`cosf`/`powf`/`expf` in `FX_Engine.h` is software-emulated, and so is every plain `float` add and multiply. Two rules follow, both measured in the disassembly on 2026-09-04: **every floating-point literal needs its `f`** (Arduino's own `PI` is a double literal, which silently promoted 24 operations per frame to double until `PI_F` replaced it), and **a lookup table only pays off if it can be addressed with integers** — a sine table written with float interpolation was measured as *slower* than `sinf` and removed again, see the comment in `FX_Engine.h`. Measured cost as of 2026-09-04: `updateEngines` 4.3% of one core, the audio block 9.5% (FFT 3.8%), ~14% together. **The C3 is not CPU-bound**, which also settles the "different SoC" question — only the ESP32/S3/H4/P4 have an FPU, and the C5, C6 and S2 do not. Output assembly already runs at the DMX cadence rather than every loop iteration; `scripts/measure_cpu.sh` re-measures all of it, and its header explains why `engUs` and `engMax` each answer a different question.
...
**Checking the UI actually parses:** run `scripts/check_ui.sh` after editing `ui/index.html`. Besides transpiling each block it now also flags a React hook that a block **uses but does not destructure from `React` in that same block** — that transpiles fine and then throws a ReferenceError at render, blanking the tab. It caught exactly that on 2026-09-04 (useRef/useCallback added to the PATCH tab whose block had only useState/useEffect). There is no build step -- Babel runs in the browser at load time -- so nothing else catches a syntax error before it is on the device, where it shows up as a blank page. The script transpiles each `<script type="text/babel">` block with the Babel already vendored in `data/vendor`, needs only `node`, and reports blocks separately because each is its own scope. It has already caught a duplicate identifier that would have blanked the header (2026-09-01), and the same class of bug blanked the AUDIO tab once before that.
...
**Checking `data/` still fits on the device:** the LittleFS partition (`spiffs` in the partition table) is **896KB** since 2026-09-03, with **~216KB free** since the UI was gzipped on 2026-09-04, not the "~85% full" this file claimed until then (that figure was stale and had been discouraging UI additions for no reason). Current contents are ~680KB (`index.html.gz` 62KB + `data/vendor/*.gz` 594KB, of which Babel alone is 548KB + `scenes.json` ~7KB). Measured, not estimated: `pio run -t buildfs` builds the image, then count the 4096-byte blocks in `.pio/build/supermini/littlefs.bin` that are not entirely `0xFF`/`0x00` — 199 of 352 were in use. Still run `pio run -t buildfs` after touching anything under `data/`; it fails loudly (`LFS_ERR_NOSPC`) if the contents don't fit, rather than silently truncating, and it's fast (~1s). **The budget is now roughly balanced:** as of 2026-09-04 `pio run` reports Flash at **78.6%** (1,236,345 of 1,572,864 bytes) — ~336KB of headroom, up from ~78KB before the partition change. **Flashing the filesystem wipes `/scenes.json`, and there is no longer an NVS fallback to rebuild it from.** Download the scenes first (PATCH tab, or `GET /api/scenes`) and put them back afterwards (`POST /api/scenes`, or the RESTORE button). Re-read the figure from `pio run` rather than trusting this line; it has been stale before. Note that deleting unreferenced code does not move it — the linker already drops what nothing calls, so removing dead code buys readability, not space.
...
1. **The engine field** in `FX_Engine.h` (`Modulator` / `MovementEngine` / `StepFX`) and the code
   that reads it.
2. **`SceneData`** in the `.ino` — add the field **with the default that reproduces the old
   behaviour**, as a member initialiser. That default is what an older saved file will get,
   because it simply lacks the key.
3. **`SCENE_FIELDS`**, the list right below the struct. Save and load are both generated from it,
   so they cannot drift apart, and this is the only other place a field has to appear.
   *(Storage used to be a raw struct blob in NVS whose byte count was checked on load, so every
   new field silently invalidated every saved scene and needed its own migration branch. That is
   gone — scenes are keyed JSON on LittleFS since 2026-09-04. Do not reintroduce a positional
   format.)*
4. **Scene application** — `triggerSceneFX()` / `executePreset()` / `executeChaserSlot()`.
5. **Scene save** — the `/save` handler in `WebAPI.h`, which fills the `SceneData` from the live
   FX objects. Note it default-constructs rather than `memset`s, so a field forgotten here keeps
   its proper default instead of becoming 0.
