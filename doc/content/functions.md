...
`syncBeats[]`, 0–9 — see `SYNC_BEATS_COUNT`; 16/32/64 beats are indices 7/8/9
because they were appended to keep stored scenes valid), `holdTime` (ms,
used when `trigger == 0`),
...
- **`void process(unsigned long now, float beatsElapsedTotal, const float* syncBeats, float &outVal)`**
  ...
  - `syncBeats` — pointer to the beat-division table (`syncBeats[]` in the
    `.ino`, `SYNC_BEATS_COUNT` entries: 8/4/2/1/½/¼/⅛ then 16/32/64 beats).
    Indexed by `sync`, clamped here to `SYNC_BEATS_COUNT - 1`. Note
    `MovementEngine` is handed a *different* table (`moveSyncBeats[]`,
    `MOVE_SYNC_BEATS_COUNT` entries, 1 … 128 beats) and clamps against its
    own length — the two must not be confused.
